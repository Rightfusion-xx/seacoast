						...
						<tr class="infoBoxContents">
							...
							<table border="0" width="100%" cellspacing="0" cellpadding="2">
								<tr>
									...
									$info_box_contents[0][] = array('align' => 'center', 'params' => 'class="productListing-heading"', 'text' => TABLE_HEADING_QUANTITY);
									$info_box_contents[0][] = array('align' => 'right', 'params' => 'class="productListing-heading"', 'text' => TABLE_HEADING_TOTAL);
									$info_box_contents[0][] = array('align' => 'right', 'params' => 'class="productListing-heading"', 'text' => 'Savings');
									$any_out_of_stock = 0;
									...
									for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
										...
										$info_box_contents[$cur_row][] = array('align' => 'center', 'params' => 'class="productListing-data" valign="top"', 'text' => tep_draw_checkbox_field('cart_delete[]', $products[$i]['id']));
										$products_name = '<table border="0" cellspacing="2" cellpadding="2">' . '  <tr>' . '    <td align="center">&nbsp;</td>' . '    <td class="productListing-data" valign="top"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '"><b>' . $products[$i]['name'] . '</b></a>';
										...
										$info_box_contents[$cur_row][] = array('align' => 'center', 'params' => 'class="productListing-data" valign="top"', 'text' => $products[$i]['id'] == CM_FTPID ? tep_draw_hidden_field('products_id[]', $products[$i]['id']) . '&nbsp;' : tep_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="4"') . tep_draw_hidden_field('products_id[]', $products[$i]['id']));
...
<?php }?>
								...
								<tr>
									<td colspan="5" align="right" class="main" > Shipping: <?php if($cart->show_total()>=25 && defined('MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING') && (MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING == 'true')){echo 'Ships FREE<br/>(US Lower 48)';}elseif (MODULE_SHIPPING_FLAT_STATUS=='True'){echo '$4.95 flat rate<br/>(US Lower 48)';}else{
									?><!--<a href="checkout_shipping.php" onclick="document.getElementById('ship_policy').style.display='block';return(false);">--><b style="background:yellow;">Lowest Price Guaranteed</b><?php }?>
