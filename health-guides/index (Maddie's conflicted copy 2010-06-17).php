...
if($hasCats)
{
    $cats_query = tep_db_query('select * from categories c join categories_description cd on cd.categories_id=c.categories_id where cd.categories_name like "'.$category.'%" order by categories_name asc');
}
else
{
    $cats_query = tep_db_query('select * from categories c join categories_description cd on cd.categories_id=c.categories_id where c.parent_id<1 order by sort_order asc');
}
