...
if($hasCats)
{
    $cats_query = tep_db_query('select * from ailments where ailment like "'.$category.'%" order by ailment asc');
}
else
{
    $cats_query = tep_db_query('select * from ailments order by popularity desc LIMIT 0,50;');
}
...
<?php require(DIR_WS_INCLUDES . 'header.php'); ?>
    ...
    <?php
        ...
        while($cats_db=tep_db_fetch_array($cats_query))
        {
            ?>
            <p>
                <?php echo $i;?>. <b><a href="/ailments.php?remedy=<?php echo urlencode(strtolower($cats_db['ailment']));?>"><?php echo $cats_db['ailment']?></a></b>
                <br />
                Health guides and detailed information on <b><?php echo $cats_db['ailment']?></b>.
