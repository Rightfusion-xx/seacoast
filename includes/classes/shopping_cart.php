...
class shoppingCart
{
    ...
    public function save_contents()
    {
        ...
        /*$saved_id = tep_db_fetch_array(tep_db_query('SELECT
                    `saved_id`
                FROM `customers_basket_saved`
                WHERE customer_id = \'' . $customer_id . '\' AND `saved_ordered` = \'no\''));
        if(!empty($saved_id))
        {
            ...
            tep_db_query('
                ...
                SET
                    `saved_content` = \'' . json_encode($this->contents) . '\'
                WHERE
                    `saved_id` = \'' . $saved_id . '\'
            ');
        }
    ...
    public function get_saved_contents($code)
    {
        $saved = tep_db_fetch_array(tep_db_query('SELECT
                    *
                FROM `customers_basket_saved`
                WHERE saved_code = \'' . $code . '\''));
        if(empty($saved))
        {
            return false;
        }

        $saved['products'] = $this->get_products(json_decode($saved['saved_content'], true));
        return $saved;
    }
