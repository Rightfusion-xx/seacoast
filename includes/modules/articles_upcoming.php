...
?>
<?php
  $expected_query = tep_db_query("select a.articles_id, a.articles_date_added, 
  a.articles_date_available as date_expected, ad.articles_name, 
  ad.articles_head_desc_tag, au.authors_id, au.authors_name, 
  td.topics_id, td.topics_name from " . TABLE_ARTICLES . " a join
   " . TABLE_ARTICLES_TO_TOPICS . " a2t on a2t.articles_id=a.articles_id
   join " . TABLE_TOPICS_DESCRIPTION . " td on td.topics_id=a2t.topics_id
   join " . TABLE_AUTHORS . " au on a.authors_id = au.authors_id
   join " . TABLE_ARTICLES_DESCRIPTION . " ad on a.articles_id = ad.articles_id 
   where to_days(a.articles_date_available) > to_days(now()) 
   and  a.articles_status = '1' and ad.language_id = '" . (int)$languages_id . "' and td.language_id = '" . (int)$languages_id . "' order by date_expected limit " . MAX_DISPLAY_UPCOMING_ARTICLES);
