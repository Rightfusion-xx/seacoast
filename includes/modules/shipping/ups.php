...
// class methods
    function quote($method = '') {
      ...
      if ( (is_array($upsQuote)) && (sizeof($upsQuote) > 0) ) {
		...
		'cost' => (MODULE_SHIPPING_UPS_HANDLING + $cost) * $shipping_num_boxes);
		}
        //for ($i=0; $i<$qsize; $i++) {
          //list($type, $cost) = each($upsQuote[$i]);
          //$methods[] = array('id' => $type,
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('UPS Packaging?', 'MODULE_SHIPPING_UPS_PACKAGE', 'CP', 'CP - Your Packaging, ULE - UPS Letter, UT - UPS Tube, UBE - UPS Express Box', '6', '6', now())");
