...
// Add the session ID when moving from different HTTP and HTTPS servers, or when SID is defined
    ...
    $link=str_replace('&sort=1a','',$link);
    
    foreach(preg_split('/&/',$link) as $item)
    {
      if($item=='page=1')
      {
...
// Outputs a button in the selected language
  function tep_image_submit($image, $alt = '', $parameters = '') {
    ...
    $image_submit = '<input type="image" src="' . tep_output_string(str_replace($_SERVER['DOCUMENT_ROOT'],'',DIR_WS_LANGUAGES) . $language . '/images/buttons/' . $image) . '" border="0" alt="' . tep_output_string($alt) . '"';
