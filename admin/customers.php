...
<?php
  if ($action == 'edit' || $action == 'update') {
    ...
    $renew_array = array(array('id' => '1', 'text' => 'Auto-Renew On'),
                              array('id' => '0', 'text' => 'Auto-Renew Off'));
?>
      <tr>
...
?></td>
      ...
      <tr>
        <td class="formAreaTitle"><?php echo 'Community Membership'; ?></td>
      </tr>
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
            <td class="main"><?php echo 'Community Membership'; ?></td>
            <td class="main">
				<?php
				  if ($processed == true) {
				    if ($cInfo->cm_renew == '1') {
				      echo 'Renew Membership';
				    } else {
				      echo 'Do Not Renew Membership';
				    }
				    echo tep_draw_hidden_field('cm_renew');
				  } else {
				    echo tep_draw_pull_down_menu('cm_renew', $renew_array, (($cInfo->cm_renew == '1') ? '1' : '0'));
				  }
				?></td>
          </tr>
          <tr>
            <td class="main"><?php echo 'Membership Expiration'; ?></td>
            <td class="main">
			<?php
			  if ($processed == true) {
			    echo $cInfo->cm_expiration . tep_draw_hidden_field('cm_expiration');
			  } else {
			    echo tep_draw_input_field('cm_expiration', $cInfo->cm_expiration, 'maxlength="32"');
			  }
			?></td>
			</tr>
<tr>
            <td class="main"><?php echo 'Membership Price'; ?></td>
            <td class="main">
			<?php
			  if ($processed == true) {
			    echo $cInfo->cm_price . tep_draw_hidden_field('cm_price');
			  } else {
			    echo tep_draw_input_field('cm_price', $cInfo->cm_price, 'maxlength="32"');
			  }
			?></td>
			</tr>
			</table></td>
...
<?php
    ...
    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_email_address, a.entry_country_id from " . TABLE_CUSTOMERS . " c left outer join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id " . $search . " order by c.customers_lastname, c.customers_firstname";
