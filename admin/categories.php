 ...
 * By: AH (GCS)
 * On: 23 January 2012
 * Reason: To fix the expire date over write issue.
 */
...
if ($_REQUEST['inv'] == 'update') {
	// code for updating product details (quantity,expiration,location) through right side pane
	// on main products listing (AH 23 Jan 2012)
...
if (isset($_REQUEST['products_location']) && isset($_REQUEST['pID'])) {
	$prev_prod_loc = tep_db_fetch_array(tep_db_query('select * from products_location where products_id=' . (int)$_REQUEST['pID'] . ' order by time_created desc limit 0,1'));

	if ($prev_prod_loc['location'] != $_REQUEST['products_location']) {
		//add to db
		tep_db_query('insert into products_location(products_id, location) values(' . (int)$_REQUEST['pID'] . ',"' . $_REQUEST['products_location'] . '")');
	}
}
	...
	switch ($action) {
		...
		case 'update_product' :
			if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
				...
				// format expiry date, if an invalid value is entered for date, leave the date unchanged (AH 23/01/2012)
								
				$date = DateTime::createFromFormat('Y-m-d', $_REQUEST['products_nearest_expiration_date']);
				$datestr = is_object($date) ? $date -> format('Y-m-d') : 'products_nearest_expiration_date';
				...
				$sql_data_array = array('products_quantity' => tep_db_prepare_input($HTTP_POST_VARS['products_quantity']), 
					...
					'products_nearest_expiration_date' => $datestr, // assign the formatted date (AH 23/01/2012)
				...
				if ($action == 'insert_product') {
					unset($sql_data_array['products_ailments']);
					unset($sql_data_array['products_uses']);
					unset($sql_data_array['products_departments']);
					unset($sql_data_array['products_keywords']);
...
default: $in_status = true; $out_status = false;
				...
				<script language="javascript">
					<!--
var dateAvailable = new ctlSpiffyCalendarBox("dateAvailable", "new_product", "products_date_available","btnDate1","<?php echo $pInfo -> products_date_available;?>
...
?>
	...
	//-->
				</script><?php echo tep_draw_form('new_product', FILENAME_CATEGORIES, 'cPath=' . $cPath . (isset($HTTP_GET_VARS['pID']) ? '&pID=' . $HTTP_GET_VARS['pID'] : '') . '&action=new_product_preview', 'post', 'enctype="multipart/form-data"');?>
				<table border="0" width="100%" cellspacing="0" cellpadding="2">
					...
					<tr>
						...
						<table border="0" cellspacing="0" cellpadding="2">
							...
							<tr>
								<td class="main"><?php echo 'Kill Product:';?></td>
								<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_checkbox_field('products_die', '', $pInfo -> products_die);?></td>
							</tr>
							<tr>
								<td class="main"><?php echo 'Orders \'til Dead:';?></td>
								<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_dieqty', $pInfo -> products_dieqty);?></td>
							</tr>
							<tr>
								<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10');?></td>
							</tr>
...
<?php
	if ($i == 0)
		echo TEXT_PRODUCTS_NAME;
			...
			<tr>
				<td class="main"> Target Keyword (Simple Product Name) </td>
				<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_target_keyword', $pInfo -> products_target_keyword);?></td>
			</tr>
			<tr>
				<td class="main" valign="top"><?php  echo 'Product "Takeaway"';?></td>
				<td>
			...
			<tr>
				<td class="main"><?php echo 'UPC:'
				?></td>
				<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_upc', $pInfo -> products_upc);?></td>
			</tr>
			<tr>
				<td class="main"><?php echo 'SKU:'
				?></td>
				<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_sku', $pInfo -> products_sku);?></td>
			</tr>
			<!--  Adding this new field to display the expiry date of the product and to interact with it -->
			<!--  AH 23 Jan 2012  -->
			<tr>
				<td class="main"><?php echo 'Nearest Expiration Date';?></td>
				
				<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;';?>
					<script language="javascript">
						<!--
						var expirydate = new ctlSpiffyCalendarBox("expirydate", "new_product", "products_nearest_expiration_date","btnDate2",
						"<?php echo $pInfo -> products_nearest_expiration_date;?>",scBTNMODE_CUSTOMBLUE);
						-->
					</script>	
					<script language="javascript">
						expirydate.writeControl();
 ...
 ?></td> <td>
			...
			<tr>
				<td colspan="3" class="main"><?php echo tep_draw_textarea_field('products_description[' . $languages[$i]['id'] . ']', 'soft', '110', '40', (isset($products_description[$languages[$i]['id']]) ? $products_description[$languages[$i]['id']] : tep_get_products_description($pInfo -> products_id, $languages[$i]['id'])), 'id="products_description"');?></td>
			</tr>
			<?php
			}
			?>
			...
			<td class="main" valign="top"><?php  echo 'Product Types (e.g. 5-HTP, Vitamin B, Omega-3):';?></td>
			<td>
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="main" valign="top">&nbsp;</td>
					<td class="main"><?php echo tep_draw_input_field('products_keywords', $pInfo -> products_keywords, 'size="50"');?></td>
				</tr>
			</table>
			<tr>
				<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10');?></td>
			...
			<tr>
				...
				<hr>
				<?php echo TEXT_PRODUCT_METTA_INFO;?></td>
			</tr>
			<tr>
				<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10');?></td>
			</tr><?php
...
<td class="main" valign="top"><?php
	if ($i == 0)
		echo TEXT_PRODUCTS_PAGE_TITLE;
			...
			?>
			<tr>
				<td class="main" valign="top"><?php  echo 'Ailments:';?></td>
				<td>
				<table border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td class="main" valign="top">&nbsp;</td>
						<td class="main"><?php echo tep_draw_textarea_field('products_ailments', 'soft', '70', '5', $pInfo -> products_ailments);?></td>
					</tr>
				</table>
				<tr>
					<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10');?></td>
				...
				<tr>
					<td class="main" valign="top"><?php  echo 'Uses:';?></td>
					<td>
					<table border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td class="main" valign="top">&nbsp;</td>
							<td class="main"><?php echo tep_draw_textarea_field('products_uses', 'soft', '70', '5', $pInfo -> products_uses);?></td>
						</tr>
					</table>
					<tr>
						<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10');?></td>
					...
					<tr>
						<td class="main" valign="top"><?php  echo 'Departments:';?></td>
						<td>
						<table border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td class="main" valign="top">&nbsp;</td>
								<td class="main"><?php echo tep_draw_textarea_field('products_departments', 'soft', '70', '5', $pInfo -> products_departments);?></td>
							</tr>
						</table>
						<tr>
							<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10');?></td>
					...
					<tr>
						<td class="main"><?php echo 'Qty Available';?></td>
						<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_available', $pInfo -> products_available);?></td>
					</tr>
					<tr>
						<td class="main"><?php
						...
						<td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_location', $location_log['location']);?></td>
					</tr>
					<tr>
						<td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10');?></td>
					</tr>
...
<?php
	if ($i == 0)
		...
		$pInfo->products_ailments = tep_get_products_url($pInfo->products_ailments, $languages[$i]['id']);
		$pInfo->products_uses = tep_get_products_url($pInfo->products_uses, $languages[$i]['id']);
		$pInfo->products_departments = tep_get_products_url($pInfo->products_departments, $languages[$i]['id']);
		$pInfo->products_keywords = tep_db_prepare_input($pInfo->products_keywords);
		} else {
		...
		$pInfo->products_ailments = tep_db_prepare_input($products_ailments);
		$pInfo->products_uses = tep_db_prepare_input($products_uses);
		$pInfo->products_departments = tep_db_prepare_input($products_departments);
		$pInfo->products_keywords = tep_db_prepare_input($pInfo->products_keywords);
			...
			<tr>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						...
						if ($new_price != '') {$pricing .= '<td class="oldPrice"';
						} else {$pricing .= '<td';
						}
						$pricing .= ' align=right>' . $currencies -> display_price($pInfo -> products_price, tep_get_tax_rate($pInfo -> products_tax_class_id)) . '</td></tr>';
						if ($new_price != '') {$pricing .= '<tr class="specialPrice"><td>' . TEXT_PRODUCTS_SALE . '</td><td align=right>' . $currencies -> display_price($new_price, tep_get_tax_rate($pInfo -> products_tax_class_id)) . '</td></tr>';
						}
						if ($pInfo -> products_msrp > $pInfo -> products_price) {
							if ($new_price != '') {$pricing .= '<tr><td>' . TEXT_PRODUCTS_SAVINGS . '</td><td align=right>' . $currencies -> display_price(($pInfo -> products_msrp - $new_price), tep_get_tax_rate($pInfo -> products_tax_class_id)) . '</td></tr>';
							} else {$pricing .= '<tr><td>' . TEXT_PRODUCTS_SAVINGS . '</td><td align=right>' . $currencies -> display_price(($pInfo -> products_msrp - $pInfo -> products_price), tep_get_tax_rate($pInfo -> products_tax_class_id)) . '</td></tr>';
							}
						} else {
							if ($new_price != '') {$pricing .= '<tr><td>' . TEXT_PRODUCTS_SAVINGS . '</td><td align=right>' . $currencies -> display_price(($pInfo -> products_price - $new_price), tep_get_tax_rate($pInfo -> products_tax_class_id)) . '</td></tr>';
							}
						}
						$pricing .= '</table>';
						?>
						...
						?>
						<tr>
						<td align="center" class="smallText"><?php echo sprintf(TEXT_PRODUCT_DATE_ADDED, tep_date_long($pInfo -> products_date_added));?>
						...
						<td class="dataTableContent" align="right"><?php
							if (isset($cInfo) && is_object($cInfo) && ($categories['categories_id'] == $cInfo -> categories_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', '');
							} else { echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $categories['categories_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
							}
 ?>&nbsp;</td>
						</tr>
						<?php
						...
						echo ' <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $products['products_id']. '&search=' . $_REQUEST['search']) . '\'">' . "\n";
						}
						?>
						<td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $products['products_id'] . '&action=new_product_preview&read=only&search=' . $_REQUEST['search']) . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $products['products_name'];?></td>
						...
						?></td>
						<td class="dataTableContent" align="right"><?php
							if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo -> products_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', '');
							} else { echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>';
							}
 ?>&nbsp;</td>
						</tr>
						<?php
 ...
 ?>&nbsp;</td>
							...
							default :
								if ($rows > 0) {
									if (isset($cInfo) && is_object($cInfo)) {// category info box contents
										...
										$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo -> products_id . '&action=new_product') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo -> products_id . '&action=move_product') . '">' . tep_image_button('button_move.gif', IMAGE_MOVE) . '</a> <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo -> products_id . '&action=copy_to') . '">' . tep_image_button('button_copy_to.gif', IMAGE_COPY_TO) . '</a>');
										if (strlen($pInfo -> sID) > 0) {
											$contents[] = array('text' => '<br>' . '<a href="specials.php?sID=' . $pInfo -> sID . '" style="color:red;font-weight:bold;text-decoration:underline;">On Special</a>' . ' ');
										}

										$contents[] = array('text' => '<br>' . 'Manufacturer:' . ' ' . $pInfo -> manufacturers_name);
										$contents[] = array('text' => '' . 'SKU:' . ' ' . $pInfo -> products_sku);
						...
						');
										...
										$contents[] = array('text' => '<br>' . 'Location' . ' ' . '<input size="7" type="text" name="products_location" value="' . $location_log['location'] . '"><br/><br/><input type="submit" value="Update"><input type="hidden" name="inv" value="update"></form>');
										$loc_log = '<br/><b>Previous Locations</b>';
										do {
											$loc_log .= '<br/>' . $location_log['location'];

										} while($location_log=tep_db_fetch_array($location_query));
										$contents[] = array('text' => $loc_log);
