<?php
	...
	tep_db_query('create table stats_3060
					select products_id, 0 as products_quantity_inprocess, 0 as products_quantity,
						000.000000 as avg_dailysales, 0 as daystoexhaustion, 
	...
	tep_db_query('update stats_3060 set 
					products_quantity_inprocess=0;
					');
	tep_db_query('update stats_3060 set products_quantity=(select case when products_quantity_inprocess=0 then products_available 
                             else products_available-products_quantity_inprocess end  from products p where p.products_id=stats_3060.products_id);');
	tep_db_query('update stats_3060 set daystoexhaustion=case when (products_quantity/avg_dailysales)<0 then 0 else 
                             (products_quantity/avg_dailysales) end;');
	tep_db_query('update stats_3060 set four_week_needs=case when (avg_dailysales*30-products_quantity)/3<3 then 3 else 
                             (avg_dailysales*30-products_quantity)-mod(avg_dailysales*30-products_quantity, 3) end;');
	tep_db_query('delete from stats_3060 where daystoexhaustion=0 and products_quantity>=0 and four_week_needs<=3;');
	
	
	$inventory_query=tep_db_query('
		select s.products_id, s.avg_dailysales, s.daystoexhaustion, s.four_week_needs, pd.products_name, p.products_sku, m.manufacturers_name,
		...
		m.manufacturers_id=p.manufacturers_id 
			...
			and (daystoexhaustion<=14 or s.products_quantity<0) and four_week_needs>=3
