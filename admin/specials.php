...
<?php
  ...
  switch ($action) {
    ...
    default:
    if($_REQUEST['sID']!=''){
        $specials_query_raw = "select p.products_image, p.products_id, pd.products_name, p.products_price, s.specials_id, s.specials_new_products_price, s.specials_date_added, s.specials_last_modified, s.expires_date, s.date_status_change, s.status from " . TABLE_PRODUCTS . " p, " . TABLE_SPECIALS . " s, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = s.products_id and specials_id='".tep_db_input($_REQUEST['sID'])."' order by pd.products_name ";
        $specials_query = tep_db_query($specials_query_raw);
