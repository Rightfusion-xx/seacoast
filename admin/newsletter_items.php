...
class actionClass extends ActionAbstract
{
    ...
    protected $_keyField = 'newsletters_id';
    protected $_pageTitle = 'Newsletters items';
    protected $_scriptUrl = '/newsletter_items.php';
    protected $_gridFields = array(
        'category_title' => 'Category',
        'title'      => 'Newsletter Title',
        'date_added' => 'Newsletter Added',
    );
    ...
    protected function _getGridReq()
    {
        return '
            ...
            LEFT JOIN `newsletter_categories` AS nc ON (t.category_id = nc.category_id)
    ...
    public function getForm($data)
    {
        ...
        {
            $options .= '<option value="' . $row['category_id'] . '"' . (($data['category_id'] == $row['category_id'])?' selected="selected"':'') . '>' . $row['category_title'] . '</option>';
        }
        ...
        return '
            <table width="100%">
                <tr>
                    <td>Title:</td>
                    <td>
                        <input type="text" name="title" style="width:100%" value="' . htmlentities($data['title']) . '" />
                    </td>
                </tr>
                <tr>
                    <td>
                ...
                <tr>
                    <td>Description:</td>
                    <td>
                        <textarea id="content' . $sign . '" name="content" style="width:100%;height:400px;">' . htmlentities($data['content']) . '</textarea>
                    </td>
                </tr>
                <tr>
                    <td>Added at:</td>
                    <td>
                        <input type="hidden" name="date_added" value="' . $data['date_added'] . '" />
                        ' . $data['date_added'] . '
                    </td>
                </tr>
            </table>
            <script type="text/javascript">
                $(document).ready(function()
                {
                    $("#main_form").submit(function(){
                        ...
                        {
                            alert("Chose a newsletter category");
                            return false;
                        }
                    });
