...
<?php if( $HTTP_SERVER_VARS["QUERY_STRING"] > 0 ){  ?>
  <meta http-equiv="refresh" content="<?php echo $HTTP_SERVER_VARS["QUERY_STRING"];?>;URL=whos_online.php?<?php echo $HTTP_SERVER_VARS["QUERY_STRING"];?>">
<?php } ?>
<!-- WOL 1.6 EOF -->
...
/* END COUNT MOD */
   ...
   //   Check who it is and set values
    ...
    if ($whos_online['customer_id'] < 0) {
      ...
      $is_bot = true;
      // Admin detection
    } elseif ($whos_online['ip_address'] == tep_get_ip_address() ) { //$HTTP_SERVER_VARS["REMOTE_ADDR"]) {
      $total_admin++;
     ...
     if ($i>0) { $cat_list.=' / '.$category['categories_name']; } else { $cat_list=$category['categories_name']; }
		...
		//              echo $temp_url_display;
              ...
              <tr class="dataTableRow">
                ...
                if ( $HTTP_SERVER_VARS["QUERY_STRING"] == showAll ) {
								  display_details();
								}
								// Display Details for Bots
								else if( $HTTP_SERVER_VARS["QUERY_STRING"] == showBots ){
								  if ( $is_bot ) {
									  display_details();
									}
								}
								// Display Details for Customers
								else if( $HTTP_SERVER_VARS["QUERY_STRING"] == showCust ){
