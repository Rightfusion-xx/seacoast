...
/// bof Product Special Expire Date
  ...
  function tep_get_products_special_expire($product_id) {



    $special_expire = tep_db_query("select expires_date from " . TABLE_SPECIALS . " where products_id = '" . (int)$product_id . "' and status");



    $expire = tep_db_fetch_array($special_expire);



    return $expire['expires_date'];
...
// Text Mode
 ...
 return $product['products_head_title_tag'];
  ...
  function tep_get_products_departments($product_id, $language_id) {

    $product_query = tep_db_query("select products_departments from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);
    return $product['products_departments'];
  }




  function tep_get_products_ailments($product_id, $language_id) {

    $product_query = tep_db_query("select products_ailments from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);
    return $product['products_ailments'];
  }



  function tep_get_products_uses($product_id, $language_id) {

    $product_query = tep_db_query("select products_uses from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);
    return $product['products_uses'];
  }
...
{
    return(preg_replace("/[^a-zA-Z0-9s_\-[:space:]]/", "", $string));
