...
class authnet
{
    ...
    function addRefund(&$order, &$ph, $amount)
    {
      ...
      if(date("H",time())>=18)
      {
        $lastsettlement=strtotime(date("M d, Y",time()) .' 6:00pm');
      }
      else
      {
        $lastsettlement=strtotime(date("M d, Y",time()-86400) .' 6:00pm');
      }
      ...
      if(strtotime($ph['transaction_time'])>$lastsettlement)
      {
        ...
        if($amount<$ph[9])
        {
          return(Array(3=>'Cannot refund less than the full amount because the transaction has not been settled.'));
        }
        else
        {
          //do the transaction
...
function logResults(&$results)
{
  if($record=tep_db_fetch_array(tep_db_query('select * from authnet_transactions where trans_id="'.$results[6].'"')))
  {
    ...
    tep_db_query('update authnet_transactions set orders_id="'.tep_db_input($results[7]).'", results="'.tep_db_input(gzcompress(serialize($results),9)).'" where trans_id="'.tep_db_input($results[7]).'"');
  }
  else
  {
