...
if($hasCats)
{
    $cats_query = tep_db_query('select * from symptoms where right(symptom,length(symptom)-instr(symptom," ")) like "'.$category.'%" order by symptom asc');
}
else
{
    $cats_query = tep_db_query('select * from symptoms order by popularity desc LIMIT 0,50;');
}
...
<?php require(DIR_WS_INCLUDES . 'header.php'); ?>
    ...
    <?php
        ...
        while($cats_db=tep_db_fetch_array($cats_query))
        {
            ?>
            <p>
                <?php echo $i;?>. <b><a href="/natural_uses.php?use=<?php echo urlencode(strtolower($cats_db['symptom']));?>"><?php echo parse_section(trim($cats_db['symptom']).'<eof>',' ','<eof>');?></a></b>
                <br />
                Health guides and detailed information on <b><?php echo parse_section(trim($cats_db['symptom']).'<eof>',' ','<eof>');?></b>.
