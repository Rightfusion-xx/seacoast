...
//  echo "<td align=\"center\" colspan=\"2\" width=\"30%\">Actions</td></tr>";
  ...
  for($i=0; $i<mysql_numrows($r); $i++) {
    ...
    $sweepstake_id=$f['sweepstake'];
    if($sweepstake_id==''){$sweepstake_id=1;}
    ...
    echo "<td align=\"center\" rowspan=\"$num\" width=\"20%\"><a href=\"sweepstakes.php?action=0&sweepstake=$f[id]\">Enroll</a></td>";
    echo "</tr>";
...
function add_player_step1() {
  ...
  echo "<form name=\"new_player\" action=\"sweepstakes.php?action=1&sweepstake=$id\" method=\"POST\">";
...
function add_player_step2() {
  ...
  $r=mysql_query("select * from users where user_email='".mysql_escape_string($email)."'",$link);
...
function add_player_step4() {
    include_once("allergy_redirect.php");
