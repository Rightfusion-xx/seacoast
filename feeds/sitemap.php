...
// save to file
$file=fopen('products-sitemap.xml','w+');
ob_clean();
...
foreach($products as &$product)
{
    ...
    $test_url=(str_replace('//','/',"/".seo_url_title($tname)."/".$product->manufacturer->manufacturers_name."/".seo_url_title($tmisc)."/p".$product->productsdescription->products_id));
    $prod=tep_db_fetch_array(tep_db_query("SELECT * FROM products p LEFT OUTER JOIN orders_products op ON op.products_id=p.products_id
    ...
    elseif($numdays>=1 and $numdays<=29)
    {
        $changefreq='weekly';
